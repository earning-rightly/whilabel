...
from apps.batches.wb_bottler_collector.wb_bottler_collector_pre.wb_bottler_collector_pre_values import \
    bottler_summary_collect_url as url, bottler_summary_collect_data as pre_scrap
...
def collect():  # 증류소의 초기 정보를 수집하는 함수
    ...
    soup = BeautifulSoup(
        scraper.get(url=url['bottler']).content,
                    features='lxml')
    ...
    for i in tqdm(range(len(clickable_list))):  # 증류쇼 갯수만큼 반복
        pre_scrap['bottler_name'].append(clickable_list[i].text.replace("\t", "").replace("\n", ""))  # 브랜드 초기정보 수집
