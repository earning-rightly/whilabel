...
from apps.batches.wb.common.enums import BatchType, LogMode
...
def whisky_execution(batch_id: BatchType):
    ...
    write_log(current_date, current_datetime, LogMode.START.value, BatchType.BOTTER_WHISKY_LINK.value, batch_id.value)

    scrap_dict = wb_libs_func.initialize_dict(field_map['wb_whisky_link'])

    wb_whisky_collector_link_apply_func.collect(batch_type=BatchType.BOTTLER_PRE.value,
    ...
    wb_libs_func.save_to_csv(result_df, csv_path, BatchType.BOTTER_WHISKY_LINK.value)

    # JSON 파일로 저장
    json_path = f'results/{current_date}/json/link/'
    wb_libs_func.save_to_json(scrap_dict, json_path, BatchType.BOTTER_WHISKY_LINK.value)

    write_log(current_date, current_datetime, LogMode.END.value, BatchType.BOTTER_WHISKY_LINK.value, batch_id.value)
    """
    보틀러 위스키 상세 정보 수집
    """

    write_log(current_date, current_datetime, LogMode.START.value, BatchType.BOTTER_WHISKY_DETAIL.value, batch_id.value)

    scrap_dict = wb_libs_func.initialize_dict(field_map['wb_whisky_collect_detail'])

    wb_whisky_collector_detail_func.collect(BatchType.BOTTER_WHISKY_LINK.value, current_date, scrap_dict)
    ...
    wb_libs_func.save_to_json(transform_result_dict, json_path, BatchType.BOTTER_WHISKY_DETAIL.value)

    write_log(current_date, current_datetime, LogMode.END.value, BatchType.BOTTER_WHISKY_DETAIL.value, batch_id.value)
