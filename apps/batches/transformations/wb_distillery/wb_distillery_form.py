...
def replace_extracted_data_with_wb_distillery_formmat() -> [list, dict]:
    """
    ...
    distillery_detail_data = pd.read_csv(
        "/Users/choejong-won/PycharmProjects/whiskey_search/batches/results/" + wb_libs_func.extract_time()[
            0] + '/csv/detail/wb_distillery_collector_detail.csv')
    distillery_summary_data = pd.read_csv(
        "/Users/choejong-won/PycharmProjects/whiskey_search/batches/results/" + wb_libs_func.extract_time()[
            0] + '/csv/pre/wb_distillery_collector_pre.csv')
    ...
    result_df = result_df[['wb_distillery_id', 'distillery_name', 'whiskies', 'votes', 'rating', 'link', 'company_about',
         ...
         'owner', 'specialists'
            , 'spirit_stills', 'status', 'views', 'wb_ranking', 'website', 'wishlist','batchedAt','batchId']]
    ...
    for i in range(len(result_df)):
        result_df_list.append({
            ...
            'modifiedAt': None,
            'modifier': None,
            'wbId': result_df.wb_distillery_id[i],
            'name': result_df.distillery_name[i],
            'country': result_df.country[i],
            'link': result_df.link[i],
            'wbDistillery': json.loads(result_df.loc[i, :].to_json()),
            'tasteFeature' : None
