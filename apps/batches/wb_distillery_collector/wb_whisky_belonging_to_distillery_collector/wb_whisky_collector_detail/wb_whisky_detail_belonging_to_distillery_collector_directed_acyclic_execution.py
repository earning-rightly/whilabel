...
def whisky_detail_distillery_collector_executions(mode : str = 'distillery', level : str = 'detail'):
    """
    ...
    wb_whisky_collector_detail_func.collect(mode = mode, current_date=wb_libs_func.extract_time()[0])  # 위스키 사전정보 수집 함수 호출
    wb_libs_func.save_resconvert_csv_to_json(current_date=wb_libs_func.extract_time()[0],
                                             result_dict=whisky_detail_scrap, dir_path='detail/',
                                             file_form='wb_distillery_whisky_collector_detail')  # 위스키 사전정보 저장 함수 호출
    ...
    wb_libs_func.save_resconvert_csv_to_json(current_date=wb_libs_func.extract_time()[0],
                                             ...
                                             file_form='wb_' + mode + '_whiky')  # True)  # 브랜드 사전정보 저장 함수 호출
