...
class LoginUseCase {
  ...
  Future<Pair<bool, bool>> call(SnsType snsType) async {
    // _isLogined = await loginRepository.login();
    switch (snsType) {
      case SnsType.KAKAO:
        final loginUserInfo = await KaKaoOauth().login();
        if (loginUserInfo != null) _loginUserInfo = loginUserInfo;
        await _customTokenLoginService(_loginUserInfo);

        break;
      case SnsType.INSTAGRAM:
        final loginUserInfo = await InstargramOauth().login();
        if (loginUserInfo != null) _loginUserInfo = loginUserInfo;
        await _customTokenLoginService(_loginUserInfo);

        break;
      case SnsType.GOOGLE:
        final loginUserInfo = await GoogleOauth().login();
        if (loginUserInfo != null) _loginUserInfo = loginUserInfo;
  ...
  Future<bool> _isNewbie() async {
    ...
    if (currentUser != null &&
        (currentUser.uid.isNotEmpty || currentUser.name.isNotNullOrEmpty)) {
