...
class ScanWhiskyBarcodeUseCase {
  Future<File?> resizeImage(File imageFile, String path,
      {int resizedImageHeight = 800,
      int resizedImageWidth = 1000,
      int level = 0}) async {
    img.Image? decodedImage =
        await Future(() => img.decodeImage(imageFile.readAsBytesSync()));
    // final tempDir = await getTemporaryDirectory();

    debugPrint("base image's width =>>   ${decodedImage?.width}");
    debugPrint("base image's height =>>   ${decodedImage?.height}");

    if (decodedImage == null) throw Exception('decodedImage가 null입니다');
    ...
    img.Image cropImage = img.copyCrop(
      decodedImage,
      x: cropX,
      // 위에서 가장 왼쪽 꼭지점 (0,0), x축 시작점
      y: cropY,
      // 위에서 가장 왼쪽 꼭지점 (0,0), y축 시작점
      width: cropWidthLength < 0 ? 1000 : cropWidthLength,
      // x축 시작점에서 x축으로 이동할 거리
      height:
          cropHeightLength < 0 ? 800 : cropHeightLength, // y축 시작점에서 y축으로 이동할 거리
    );
    try {
      img.Image thumbnail = img.copyResize(cropImage,
          height: decodedImage.height > resizedImageHeight
              ? resizedImageHeight
              : decodedImage.height,
          width: decodedImage.width > resizedImageWidth
              ? resizedImageWidth
              : decodedImage.width);
      ...
      File savedPngImage =
          File("${path}/${Timestamp.now().millisecondsSinceEpoch}.png")
            ..writeAsBytes(unit8ListPng);
      // File savedPngImage = File.fromRawPath( unit8ListPng);
      debugPrint("\n\nresult ==> ${savedPngImage.path}\n---");
      return savedPngImage;
    } catch (e) {
      throw Exception("resize 이미지를 저장할 수 없습니다");
    }
  }
